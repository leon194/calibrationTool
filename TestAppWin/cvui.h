...
namespace internal
{
	...
	bool button(cvui_block_t& theBlock, int theX, int theY, int theWidth, int theHeight, const cv::String& theLabel, bool theUpdateLayout) {
		...
		if (aMouseIsOver) {
			...
			render::button(theBlock, cvui::OUT_s, aRect, theLabel);
			render::buttonLabel(theBlock, cvui::OUT_s, aRect, theLabel, aTextSize);
		}
		...
		return (aMouseIsOver && internal::gMousePressed) || wasShortcutPressed;
	}
	...
	bool button(cvui_block_t& theBlock, int theX, int theY, cv::Mat& theIdle, cv::Mat& theOver, cv::Mat& theDown, bool theUpdateLayout) {
		...
		switch (aStatus) {
		case cvui::OUT_s: render::image(theBlock, aRect, theIdle); break;
	...
	bool checkbox(cvui_block_t& theBlock, int theX, int theY, const cv::String& theLabel, bool *theState, unsigned int theColor) {
		...
		if (aMouseIsOver) {
			...
			render::checkbox(theBlock, cvui::OUT_s, aRect);
		}
	...
	bool trackbar(cvui_block_t& theBlock, int theX, int theY, int theWidth, long double *theValue, const TrackbarParams & theParams) {
		...
		render::trackbar(theBlock, aMouseIsOver ? OVER : OUT_s, aContentArea, *theValue, theParams);
...
namespace render
{
	...
	void button(cvui_block_t& theBlock, int theState, cv::Rect& theShape, const cv::String& theLabel) {
		...
		// Inside
		theShape.x++; theShape.y++; theShape.width -= 2; theShape.height -= 2;
		cv::rectangle(theBlock.where, theShape, theState == OUT_s ? cv::Scalar(0x42, 0x42, 0x42) : (theState == OVER ? cv::Scalar(0x52, 0x52, 0x52) : cv::Scalar(0x32, 0x32, 0x32)), CVUI_FILLED);
	}
	...
	void checkbox(cvui_block_t& theBlock, int theState, cv::Rect& theShape) {
		// Outline
		cv::rectangle(theBlock.where, theShape, theState == OUT_s ? cv::Scalar(0x63, 0x63, 0x63) : cv::Scalar(0x80, 0x80, 0x80));
