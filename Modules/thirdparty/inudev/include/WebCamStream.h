...
namespace InuDev 
{
    ...
    public:
        ...
        enum EOutputType
        {
            eDefault= 0,  	///<  Provides CImageFrames of CImageFrame::EFormat::eBGRA format
            eRegisteredWebcam = 1,  ///<  Projection of webcam image on depth, format is similar to eDefault 
            eUndistortedWebcam = 2,  ///<  Undistorted webcam image, format is similar to eDefault 
        ...
        typedef std::function<void(std::shared_ptr<CWebCamStream>, std::shared_ptr<const CImageFrame>,  CInuError)> CallbackFunction;
        ...
        virtual CInuError GetFrame(std::shared_ptr<const CImageFrame>& oWebCamFrame, unsigned int iTimeout = FPS_BASED_TIMEOUT) = 0;
