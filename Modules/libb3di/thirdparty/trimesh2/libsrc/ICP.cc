...
// max iteration numbers for different speed settings
#define MAX_ITERS_2 100
#define MAX_ITERS_1 100
#define MAX_ITERS_0 15
#define MIN_PAIRS 25
			     ...
			     vector<PtPair> &pairs, bool flip)
{
	...
	while (1) {
		...
		if (g_icpSpeed>1) {
			...
			match = kd2->closest_to_pt(p, maxdist2, &nc);
#ifdef ECXT
		} else {
			match = kd2->closest_to_pt(p, maxdist2, NULL);
		}
#endif
		if (!match)
			continue;
...
#ifndef ECXT
		// skip the boundary checking because it is slow
...
static float ICP_iter(TriMesh *s1, TriMesh *s2, const xform &xf1, xform &xf2,
...
#ifdef ECXT
			  const vector<char> &weights1, const vector<char> &weights2,
#else
		      ...
		      bool do_scale, bool do_affine, int desiredPairs)
{
	// Compute pairs
	timestamp t1 = now();
	if (verbose > 1)
...
//	incr *= (float) pairs.size() / DESIRED_PAIRS;
	incr *= (float) pairs.size() / desiredPairs;
...
#ifndef ECXT
// EC: isn't the weights always 1 or 0? no need to multiple by 1
...
static float ICP_p2pt(TriMesh *s1, TriMesh *s2, const xform &xf1, xform &xf2,
		      ...
		      float &maxdist, int verbose,
		      vector<float> &sampcdf1, vector<float> &sampcdf2,
		      float &incr, bool trans_only, int desiredPairs)
{
	// Compute pairs
	timestamp t1 = now();
	if (verbose > 1)
...
/*
		if (verbose==1) {
			...
			dprintf("Rotation %.2f %.2f %.2f\n", B[0]*180.0/pi, B[1]*180.0/pi, B[2]*180.0/pi);
		}
*/
	}
...
float ICP(TriMesh *s1, TriMesh *s2, const xform &xf1, xform &xf2,
	  ...
	  const KDtree *kd1, const KDtree *kd2,
#ifdef ECXT
	  vector<char> &weights1, vector<char> &weights2,
#else
...
#ifdef ECXT
	g_icpSpeed=speed;
	bool p2pt_only=(g_icpSpeed==0);
	int maxIters;
	if (g_icpSpeed==0)
		maxIters=MAX_ITERS_0;
	else if (g_icpSpeed==1)
		maxIters=MAX_ITERS_1;
	else
		maxIters=MAX_ITERS_2;

	timestamp t0=now();
	// Make sure we have everything precomputed
	s1->need_normals();
	
	// only need the second set of normals when the speed mode is greater than one
	if (g_icpSpeed>1)
		s2->need_normals();

// neighbor computation is slow and they are only needed in isBdy() to check if a point is in the boundary
// not worth the cost and doesn't seem to affect the convergence much without them
#else
	// Make sure we have everything precomputed
...
#ifdef ECXT
	if (verbose > 1) {
		dprintf("Time for initial data creation: %.2f msec.\n\n",
		       (t-t0) * 1000.0);
	}
#endif
	if (maxdist <= 0.0f) {
	...
	for (int i = 0; i < 2; i++) {
		if ((err=ICP_p2pt(s1, s2, xf1, xf2,
#ifdef ECXT
				ctr1, ctr2,
#endif
				kd1, kd2, maxdist, verbose,
			     sampcdf1, sampcdf2, incr, true, DESIRED_PAIRS_EARLY)) < 0.0f)
			return -1.0f;
	}
...
#endif
				kd1, kd2, maxdist, verbose,
			     sampcdf1, sampcdf2, incr, false, DESIRED_PAIRS_EARLY)) < 0.0f)
			return -1.0f;
	}
	if (verbose==1) {
		dprintf("After p2pt iterations error %.4f xform=\n", err);
		cout << xf2;
	}

#ifdef ECXT
	// skip this step if we are only doing p2pt ICP (speed is 0)
	if (!p2pt_only) {
#endif
		...
		err = ICP_iter(s1, s2, xf1, xf2,
#ifdef ECXT
					ctr2,
#endif
					kd1, kd2, weights1, weights2,
					 maxdist, verbose, sampcdf1, sampcdf2,
					 incr, true, false, false, DESIRED_PAIRS);
		if (verbose > 1) {
			timestamp tnow = now();
...
#ifdef ECXT
		if (verbose==1) {
			dprintf("After initial iterations error %.4f xform=\n", err);
			cout << xf2;
		}
	} else {
		incr *= (float) DESIRED_PAIRS_EARLY / DESIRED_PAIRS;
	}
#endif
	if (err < 0.0f)
		return err;
	...
	do {
...
#ifdef ECXT
		if (p2pt_only) {
			err=ICP_p2pt(s1, s2, xf1, xf2, ctr1, ctr2, kd1, kd2, maxdist, verbose,
			     sampcdf1, sampcdf2, incr, false, DESIRED_PAIRS);
		} else {
			bool recompute = (g_icpSpeed>1) && (iters % 10 == 9);
			if (recompute)
				compute_overlaps(s1, s2, xf1, xf2, kd1, kd2,
						 weights1, weights2, maxdist, verbose);
			err = ICP_iter(s1, s2, xf1, xf2, ctr2, kd1, kd2, weights1, weights2,
					   maxdist, verbose, sampcdf1, sampcdf2, incr,
					   recompute, do_scale && !rigid_only,
					   do_affine && !rigid_only, DESIRED_PAIRS);
		...
		if (verbose==1) {
			dprintf("After %d iterations error %.4f xform=\n", iters, err);
			cout << xf2;
		}
		if (err < 0.0f)
			return err;
	...
	if (p2pt_only) {
		err=ICP_p2pt(s1, s2, xf1, xf2, ctr1, ctr2, kd1, kd2, maxdist, verbose,
			    sampcdf1, sampcdf2, incr, false, DESIRED_PAIRS_FINAL);

	} else {
		err = ICP_iter(s1, s2, xf1, xf2, ctr2, kd1, kd2, weights1, weights2,
				   maxdist, verbose, sampcdf1, sampcdf2, incr,
				   false, do_scale, do_affine, DESIRED_PAIRS_FINAL);
	}
#else
	err = ICP_iter(s1, s2, xf1, xf2, kd1, kd2, weights1, weights2,
				maxdist, verbose, sampcdf1, sampcdf2, incr,
	...
	if (verbose==1) {
		dprintf("After final iteration error %.4f xform=\n", err);
		cout << xf2;
	}
	return err;
}
...
#ifdef ECXT
	vector<char> weights1, weights2;
	point ctr1, ctr2;
	float icperr = ICP(s1, s2, xf1, xf2, ctr1, ctr2, kd1, kd2,
