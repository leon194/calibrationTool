...
#ifdef ECXT
// For OBJ .mtl file
const string MTL_MATERIAL_NAME = "material_0";
#endif
...
#ifdef ECXT
static bool write_obj(TriMesh *mesh, FILE *f, bool write_norm, const char* mtlFileName=NULL);
#else
...
static bool write_verts_asc(TriMesh *mesh, FILE *f,
	...
	const char *before_texture = NULL);
#else
	const char *after_line);
#endif
...
static bool read_obj(FILE *f, TriMesh *mesh)
{
	...
	while (1) {
		...
		if (LINE_IS("v ") || LINE_IS("v\t")) {
			...
			mesh->textcoords.push_back(vec2(u, v));
#endif
...
bool TriMesh::write(const char *filename)
{
	...
	while (1) {
		...
		else if (begins_with(filename, "texture:")) {
			filename += 8;
			write_texture = true;
#endif
		} else {
...
#ifdef ECXT
		if (write_texture) {
			// construct the material file name from filename + ".mtl"
			string fileName(filename);
			int strIndex = fileName.find_last_of("/");
			if (strIndex == std::string::npos)
				strIndex = fileName.find_last_of("/\\");
			string base_filename = fileName.substr(strIndex + 1);
			string mtlFileName = base_filename + ".mtl";
			ok = write_obj(this, f, write_norm, mtlFileName.c_str());
		}
		else {
...
// Write a obj file
#ifdef ECXT
static bool write_obj(TriMesh *mesh, FILE *f, bool write_norm, const char* mtlFileName)
#else
static bool write_obj(TriMesh *mesh, FILE *f, bool write_norm)
#endif
{
...
#ifdef ECXT
	// Assume the texture map info is stored in a file called material.mtl
	if (mtlFileName && mesh->textcoords.size()) {
		fprintf(f, "mtllib %s\n", mtlFileName);
		if (!write_verts_asc(mesh, f, "v ", write_norm ? "\nvn " : 0, 0, false, 0, "", "\nvt "))
			return false;
		fprintf(f, "usemtl %s\n", MTL_MATERIAL_NAME.c_str());
	...
	if (!write_norm) {
		...
		if (mtlFileName && mesh->textcoords.size()) {
			for (size_t i = 0; i < mesh->faces.size(); i++) {
				...
				int n = fprintf(f, "f %d/%d/%d %d/%d/%d %d/%d/%d\n",
					v0, v0, v0,
					v1, v1, v1,
					v2, v2, v2);
				if (n < 9) {
...
#ifdef ECXT
		if (!mesh->textcoords.empty() && before_texture) {
			FPRINTF(f, "%s%.7g %.7g", before_texture, mesh->textcoords[i][0], mesh->textcoords[i][1]);
		}
#endif
		FPRINTF(f, "%s\n", after_line);
	}
