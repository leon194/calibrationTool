...
public:
    ...
    // bgFill controls how to fill the missing depth values. The default is no fill (no depth value)
    ...
    // bgColor defines the color for background pixels
    virtual bool renderPhongImage(const PhongParams& phongParams,
        cv::Mat& outputImage, 
        const cv::Vec3d& lightDirection=cv::Vec3d(0, 0, 1),
        const cv::Scalar& bgColor = 0) const;
